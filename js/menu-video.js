...
// Fonction pour démarrer la vidéo avec un délai
function restartVideoAfterDelay() {
    // Annule tout timeout précédent
    clearTimeout(videoTimeout);
    
    // Réinitialise l'état de la vidéo
    video.style.display = 'none';
    video.pause();
    video.currentTime = 0;
    video.style.opacity = 0;

    // Attendez 5 secondes avant de démarrer la vidéo
    videoTimeout = setTimeout(function () {
        video.style.display = 'block';  // Affiche la vidéo
        video.style.animation = 'fadeIn 1s forwards';  // Animation de fade-in
        video.play();  // Démarre la vidéo
    }, 1500);
}

// Fonction pour arrêter la vidéo avec une animation
function stopVideo() {
    clearTimeout(videoTimeout);  // Annule tout timeout en attente
    
    // Appliquer l'animation de fade-out
    video.style.animation = 'fadeOut 1s forwards';

    // Attendre la fin de l'animation pour vraiment cacher la vidéo
    setTimeout(function () {
        video.style.display = 'none';
        video.pause();
